...
DISPARITY_DIR = DATA_DIR / "disparity"
...
DISPARITY_LEFT_PATH = DISPARITY_DIR / "left.png"
DISPARITY_RIGHT_PATH = DISPARITY_DIR / "right.png"
...
def ensure_pinhole_dataset() -> None:
    lecture10_dir = ROOT_DIR.parent / "lecture10" / "data" / "pinhole_calib"
    for output_index, sample_id in enumerate(PINHOLE_SOURCE_IDS):
        filename = f"img_{output_index:02d}.png"
        destination = PINHOLE_DIR / filename
        if not copy_if_present(lecture10_dir / filename, destination):
...
def ensure_stereo_dataset() -> None:
    lecture10_dir = ROOT_DIR.parent / "lecture10" / "data" / "stereo" / "case1"
    for sample_id in STEREO_SOURCE_IDS:
        left_name = f"left{sample_id:02d}.png"
        right_name = f"right{sample_id:02d}.png"
        left_destination = STEREO_DIR / left_name
        right_destination = STEREO_DIR / right_name
        if not copy_if_present(lecture10_dir / left_name, left_destination):
            write_downloaded_image(
                f"https://raw.githubusercontent.com/opencv/opencv/4.x/samples/data/left{sample_id:02d}.jpg",
                left_destination,
            )
        if not copy_if_present(lecture10_dir / right_name, right_destination):
            write_downloaded_image(
...
def ensure_disparity_dataset() -> None:
    write_downloaded_image("https://raw.githubusercontent.com/opencv/opencv/4.x/samples/data/aloeL.jpg", DISPARITY_LEFT_PATH)
    write_downloaded_image("https://raw.githubusercontent.com/opencv/opencv/4.x/samples/data/aloeR.jpg", DISPARITY_RIGHT_PATH)
...
def disparity_pair_paths() -> tuple[Path, Path]:
    ensure_demo_data()
    return DISPARITY_LEFT_PATH, DISPARITY_RIGHT_PATH


def disparity_pair_images() -> tuple[np.ndarray, np.ndarray]:
    left_path, right_path = disparity_pair_paths()
    left = cv2.imread(str(left_path), cv2.IMREAD_COLOR)
    right = cv2.imread(str(right_path), cv2.IMREAD_COLOR)
    if left is None or right is None:
        raise FileNotFoundError("Could not read disparity pair images.")
    return left, right
...
def select_sample_by_name(samples: list[ChessboardSample], stem: str) -> ChessboardSample:
    for sample in samples:
        if sample.path.stem == stem:
            return sample
    return samples[-1]
...
def calibrate_stereo() -> StereoCalibration:
    ...
    left_points_list = [sample.corners for sample in left_samples]
    right_points_list = [sample.corners for sample in right_samples]
    ...
    left_points = np.stack(left_points_list)
    right_points = np.stack(right_points_list)
    np.savez(
        STEREO_RESULT_PATH,
